...
public class ApRecordServiceImpl implements ApRecordService{
    ...
    public void updateApRecord(ApRecordDTO request) throws HmsException {
        ApRecord existing = apRecordRepository.findById(request.getId()).orElseThrow(() ->
    ...
    public ApRecordDTO getApRecordByAppointmentId(Long appointmentId) throws HmsException {
        return apRecordRepository.findByAppointment_Id(appointmentId).orElseThrow(() ->
    ...
    public List<RecordDetails> getRecordsByPatientId(Long patientId) throws HmsException {
        ...
        List<Long> doctorsIds = recordDetails.stream().map(RecordDetails::getDoctorId).distinct().toList();
        List<DoctorName> doctorsNames = profileClient.getDoctorsById(doctorsIds);
        Map<Long, String> doctorMap = doctorsNames.stream()
